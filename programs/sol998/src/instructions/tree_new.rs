...
pub fn handler(ctx: Context<TreeNewAccounts>, node_bump: u8) -> Result<()> {
    // let root_pda = &mut ctx.accounts.root_node;
    ...
    node_pda.owner = user_key;
    node_pda.bump = node_bump;
    node_pda.parent_mint = None;
    node_pda.children_mint = mint;
...
#[derive(Accounts)]
pub struct TreeNewAccounts<'info> {
    #[account(
        init,
        payer = user,
...
impl<'info> TreeNewAccounts<'info> {
