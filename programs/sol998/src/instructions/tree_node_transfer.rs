...
pub fn handler(ctx: Context<TreeNodeTransferAccounts>, node_bump: u8) -> Result<()> {
...
#[derive(Accounts)]
pub struct TreeNodeTransferAccounts<'info> {
    #[account(
        mut,
        seeds = [
            NODE_PDA_SEED,
...
impl<'info> TreeNodeTransferAccounts<'info> {
